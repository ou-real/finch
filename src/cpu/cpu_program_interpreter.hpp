...
bool cpu_program_stepper(uint16_t *const maze, const uint32_t rows, const uint32_t cols,
  uint32_t *program, finch::program_state &state, uint32_t &op_lim, interpreter_state *const istate, const finch::program_state &stop);

void cpu_program_interpreter(uint16_t *const maze, const uint32_t rows, const uint32_t cols,
  uint32_t *offsets, uint32_t *programs, uint32_t program_index, finch::program_state state, uint32_t op_lim,
  finch::program_state *res, const finch::program_state &stop);
